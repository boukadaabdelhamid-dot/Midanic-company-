...
**How to apply:** Read the current remote ref and tree through the GitHub proxy. Resolve the root tree SHA from the branch's commit object, then request `/git/trees/{explicit-tree-sha}`; querying the tree by branch name returned the commit SHA in the tree response's `sha` field, while the explicit SHA returned the actual tree. Compare Git blob SHAs and paths against the regular-shell local tree snapshot, preserve the remote tree as the base, upload only changed or added blobs, and require the resulting tree SHA to match the local committed tree before a non-force branch update. In CodeExecution's impure runtime, read workspace files by relative path because `process.cwd()` is unavailable.