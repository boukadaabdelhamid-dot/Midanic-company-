...
export default function NewTransfer() {
  ...
  const [prefilledAdded, setPrefilledAdded] = useState(false);
  const prefilledProductId = Number(productIdParam);
  ...
  const prefilledProductQ = useGetProduct(prefilledProductId, {
    query: {
      enabled: ready && Number.isInteger(prefilledProductId) && prefilledProductId > 0,
      queryKey: getGetProductQueryKey(prefilledProductId),
    },
  });
  ...
  useEffect(() => {
    const product = prefilledProductQ.data;
    if (!product || prefilledAdded || product.stock <= 0) return;
    setCart((current) => current.some((line) => line.product.id === product.id)
      ? current
      : [...current, { product, quantity: 1 }]);
    setPrefilledAdded(true);
  }, [prefilledProductQ.data, prefilledAdded]);
