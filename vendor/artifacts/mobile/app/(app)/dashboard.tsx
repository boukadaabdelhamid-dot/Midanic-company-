        ...
        <View style={{ flexDirection: "row", gap: 8 }}>
          <View style={{ flex: 1 }}>
            <DateField
              ...
              onChange={(date) => onFrom(formatDateValue(date))}
              maximumDate={parseDateValue(dateTo) ?? undefined}
              placeholder={t("Choisir une date", "اختر تاريخاً")}
            />
          </View>
          <View style={{ flex: 1 }}>
            <DateField
              label={t("Fin", "النهاية")}
              value={parseDateValue(dateTo)}
              onChange={(date) => onTo(formatDateValue(date))}
              minimumDate={parseDateValue(dateFrom) ?? undefined}
              placeholder={t("Choisir une date", "اختر تاريخاً")}
            />
          </View>
        </View>
