...
router.get("/erp/purchase-orders", authenticate, requireStaff, requireStore, requirePermission("purchases", "view"), async (req: AuthRequest, res) => {
  try {
    ...
    const baseWhere = status
      ? and(eq(schema.purchaseOrdersTable.storeId, storeId), eq(schema.purchaseOrdersTable.status, status as "pending" | "received" | "cancelled"))
      : eq(schema.purchaseOrdersTable.storeId, storeId);
...
const getCustomerDetail = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const storeId = req.currentStoreId!;
    const id = pid(req, "id");

    // A customer is visible in a store only when they have a profile there or
    // at least one order in that store. This keeps legacy customers (which may
    // not have a customer_profiles row) working without exposing another
    ...
    const customerResult = await db.execute(sql`
      SELECT
        ...
        cc.label_fr AS classification_label_fr,
        cc.label_ar AS classification_label_ar,
        cc.color AS classification_color,
        cc.sort_order AS classification_sort_order,
        cp.price_tier_id,
        pt.label_fr AS price_tier_label_fr,
        pt.label_ar AS price_tier_label_ar,
        pt.code AS price_tier_code,
        pt.sort_order AS price_tier_sort_order,
        cp.account_number,
        cp.credit_limit,
        cp.min_balance_alert,
        COALESCE(
          CASE
            WHEN cp.contact_type = 'customer_supplier' AND cp.contact_id IS NOT NULL
            THEN (SELECT current_balance FROM contacts WHERE id = cp.contact_id AND store_id = ${storeId} LIMIT 1)
            ELSE NULL
          END,
          cp.current_balance,
          0
      ...
      WHERE u.id = ${id}
        ...
        AND (
          ...
          OR EXISTS (
            ...
            WHERE customer_orders.user_id = u.id
              AND customer_orders.store_id = ${storeId}
          )
        )
      LIMIT 1
    `);
    ...
    const hasClassification = customer.classification_id != null;
    const hasPriceTier = customer.price_tier_id != null;

    const profile = customer.contact_type == null
      ? null
      : {
          contactType: customer.contact_type,
          contactId: toNullableNumber(customer.contact_id),
          wilaya: customer.wilaya ?? null,
          commune: customer.commune ?? null,
          gps: customer.gps ?? null,
          classificationId: toNullableNumber(customer.classification_id),
          classification: hasClassification
            ? {
                id: toNumber(customer.classification_id),
                labelFr: String(customer.classification_label_fr ?? ""),
                labelAr: String(customer.classification_label_ar ?? ""),
                color: customer.classification_color ?? null,
                sortOrder: toNumber(customer.classification_sort_order),
              }
            : null,
          priceTierId: toNullableNumber(customer.price_tier_id),
          priceTier: hasPriceTier
            ? {
                id: toNumber(customer.price_tier_id),
                labelFr: String(customer.price_tier_label_fr ?? ""),
                labelAr: String(customer.price_tier_label_ar ?? ""),
                code: String(customer.price_tier_code ?? ""),
                sortOrder: toNumber(customer.price_tier_sort_order),
              }
            : null,
          accountNumber: customer.account_number ?? null,
          creditLimit: toNullableNumber(customer.credit_limit),
          minBalanceAlert: toNullableNumber(customer.min_balance_alert),
          currentBalance: toNumber(customer.current_balance),
          foreignCurrency: Boolean(customer.foreign_currency),
          rc: customer.rc ?? null,
          nif: customer.nif ?? null,
          ai: customer.ai ?? null,
          nis: customer.nis ?? null,
        };

    const orders = (ordersResult.rows as Array<Record<string, unknown>>).map((order) => ({
      id: toNumber(order.id),
      userId: toNullableNumber(order.user_id),
      sellerUserId: toNullableNumber(order.seller_user_id),
      sellerUser: order.seller_id == null
        ? null
        : {
            id: toNumber(order.seller_id),
      ...
      discountAmount: order.discount_amount == null ? undefined : String(order.discount_amount),
      couponCode: order.coupon_code ?? null,
      createdAt: order.created_at instanceof Date
        ? order.created_at.toISOString()
        : order.created_at == null ? undefined : String(order.created_at),
      updatedAt: order.updated_at instanceof Date
        ? order.updated_at.toISOString()
        : order.updated_at == null ? undefined : String(order.updated_at),
    }));

    const notes = (notesResult.rows as Array<Record<string, unknown>>).map((note) => ({
      id: toNumber(note.id),
      userId: toNumber(note.user_id),
      note: String(note.note ?? ""),
      createdAt: note.created_at instanceof Date
        ? note.created_at.toISOString()
        : note.created_at == null ? undefined : String(note.created_at),
    }));
    ...
    res.json({
      ...
      created_at: customer.created_at instanceof Date
        ...
        : customer.created_at == null ? undefined : String(customer.created_at),
      profile,
      orders,
      notes,
    });
...
router.get("/erp/customers/:id", authenticate, requireStaff, requireStore, requirePermission("customers", "view"), getCustomerDetail);



router.put("/erp/customers/:id", authenticate, requireStaff, requireStore, requirePermission("customers", "edit"), async (req: AuthRequest, res: Response) => {
  const body = (req.body ?? {}) as Record<string, unknown>;
  ...
  const asNullableNumber = (value: unknown, field: string): number | null => {
    ...
    if (!Number.isFinite(number)) {
      throw Object.assign(new Error(`${field} must be a valid number`), { statusCode: 400 });
    }
    return number;
  };
  ...
  try {
    ...
    const requestedName = has("name") ? String(body.name ?? "").trim() : undefined;
    ...
    let requestedContactType: "customer" | "customer_supplier" | undefined;
    if (admin && has("contactType")) {
      if (body.contactType !== "customer" && body.contactType !== "customer_supplier") {
        res.status(400).json({ error: "Invalid contact type" });
        return;
      }
      requestedContactType = body.contactType;
    }

    await db.transaction(async (tx) => {
      // Match GET visibility exactly: a customer can only be edited from a
      // store where they have a profile or an order.
      const [customer] = await tx
        .select({
          userId: schema.usersTable.id,
          name: schema.usersTable.name,
          email: schema.usersTable.email,
          phone: schema.usersTable.phone,
          address: schema.usersTable.address,
          city: schema.usersTable.city,
          profileId: schema.customerProfilesTable.id,
          profileContactId: schema.customerProfilesTable.contactId,
          profileContactType: schema.customerProfilesTable.contactType,
          profileCurrentBalance: schema.customerProfilesTable.currentBalance,
        })
        .from(schema.usersTable)
        .leftJoin(
          schema.customerProfilesTable,
          and(
            eq(schema.customerProfilesTable.userId, schema.usersTable.id),
            eq(schema.customerProfilesTable.storeId, storeId),
          ),
        )
        .where(and(
          eq(schema.usersTable.id, id),
          eq(schema.usersTable.role, "customer"),
          or(
            sql`${schema.customerProfilesTable.id} IS NOT NULL`,
            sql`EXISTS (
              SELECT 1 FROM orders customer_orders
      ...
      const name = requestedName ?? customer.name;
      const email = requestedEmail ?? customer.email;
      const phone = has("phone") ? asNullableText(body.phone) : customer.phone;
      const address = has("address") ? asNullableText(body.address) : customer.address;
      const city = has("city") ? asNullableText(body.city) : customer.city;
      ...
      if (has("name")) userUpdate.name = name;
      if (has("email")) userUpdate.email = email;
      if (has("phone")) userUpdate.phone = phone;
      if (has("address")) userUpdate.address = address;
      if (has("city")) userUpdate.city = city;
      if (Object.keys(userUpdate).length > 0) {
        await tx.update(schema.usersTable)
          .set(userUpdate)
      ...
      const contactType = requestedContactType ??
        (customer.profileContactType === "customer_supplier" ? "customer_supplier" : "customer");
      let contactId = customer.profileContactId;
      if (contactId == null) {
        const [contact] = await tx.insert(schema.contactsTable).values({
          storeId,
          name,
          ...
          address,
          contactType,
          // Preserve an existing legacy profile balance when lazily creating
          // its unified contact; this is not a client-supplied balance write.
          currentBalance: customer.profileCurrentBalance ?? "0",
          globalContactId: randomUUID(),
        }).returning({ id: schema.contactsTable.id });
        contactId = contact.id;
      } else {
        await tx.update(schema.contactsTable)
          .set({ name, email, phone, address, contactType, updatedAt: new Date() })
          .where(and(
            eq(schema.contactsTable.id, contactId),
            eq(schema.contactsTable.storeId, storeId),
          ));
      }

      const profileUpdate: {
        contactType: "customer" | "customer_supplier";
        contactId: number;
        wilaya?: string | null;
        commune?: string | null;
        gps?: string | null;
      ...
      if (has("wilaya")) profileUpdate.wilaya = asNullableText(body.wilaya);
      if (has("commune")) profileUpdate.commune = asNullableText(body.commune);
      if (has("gps")) profileUpdate.gps = asNullableText(body.gps);
      if (admin) {
        if (has("classificationId")) profileUpdate.classificationId = asNullableNumber(body.classificationId, "classificationId");
        if (has("priceTierId")) profileUpdate.priceTierId = asNullableNumber(body.priceTierId, "priceTierId");
        if (has("accountNumber")) profileUpdate.accountNumber = asNullableText(body.accountNumber);
        if (has("creditLimit")) {
          const creditLimit = asNullableNumber(body.creditLimit, "creditLimit");
          profileUpdate.creditLimit = creditLimit == null ? null : creditLimit.toFixed(2);
        }
        if (has("minBalanceAlert")) {
          const minBalanceAlert = asNullableNumber(body.minBalanceAlert, "minBalanceAlert");
          profileUpdate.minBalanceAlert = minBalanceAlert == null ? null : minBalanceAlert.toFixed(2);
        }
        if (has("foreignCurrency")) profileUpdate.foreignCurrency = body.foreignCurrency === true;
        if (has("rc")) profileUpdate.rc = asNullableText(body.rc);
        if (has("nif")) profileUpdate.nif = asNullableText(body.nif);
        if (has("ai")) profileUpdate.ai = asNullableText(body.ai);
        if (has("nis")) profileUpdate.nis = asNullableText(body.nis);
      }
      ...
      if (customer.profileId != null) {
        await tx.update(schema.customerProfilesTable)
          .set(profileUpdate)
          .where(and(
            eq(schema.customerProfilesTable.id, customer.profileId),
            eq(schema.customerProfilesTable.storeId, storeId),
          ));
      } else {
        // Deliberately omit currentBalance. Profile edits must never turn the
        // canonical GET balance into a mass-assigned role balance.
        await tx.insert(schema.customerProfilesTable).values({
          userId: id,
          storeId,
          ...profileUpdate,
      ...
      // Keep the unified contact's supplier role in step with a customer
      // promotion/downgrade without deleting historical role data.
      const [supplier] = await tx
        .select({ id: schema.suppliersTable.id })
        .from(schema.suppliersTable)
        .where(and(
          eq(schema.suppliersTable.storeId, storeId),
          eq(schema.suppliersTable.contactId, contactId),
        ))
        .limit(1);
      if (supplier) {
        await tx.update(schema.suppliersTable)
          .set({
            ...
            phone,
            address,
            contactType: contactType === "customer_supplier" ? "customer_supplier" : "supplier",
          })
          .where(and(
            eq(schema.suppliersTable.id, supplier.id),
            eq(schema.suppliersTable.storeId, storeId),
          ));
      } else if (contactType === "customer_supplier") {
        await tx.insert(schema.suppliersTable).values({
          storeId,
          name,
          email,
    ...
    // Return the same complete detail shape as GET so the mutation can be
    // rendered immediately by any caller, not just invalidated in the UI.
...
router.get(["/erp/purchases/needed", "/erp/purchases/needs"], authenticate, requireStaff, requireStore, requirePermission("purchases", "view"), async (req: AuthRequest, res) => {
  try {
    const storeId = req.currentStoreId!;
...
router.post("/erp/products/:id/expiry-batches", authenticate, requireStaff, requireStore, requirePermission("products", "edit"), async (req: AuthRequest, res) => {
  try {
    ...
    if (!dateStr || !/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
      res.status(400).json({ error: "expiryDate must be YYYY-MM-DD" });
      return;
    }
...
router.delete("/erp/expiry-batches/:batchId", authenticate, requireStaff, requireStore, requirePermission("products", "edit"), async (req: AuthRequest, res) => {
  try {
    ...
    const result = await db.execute(sql`
      ...
      RETURNING id
    `);
    if (!result.rows.length) { res.status(404).json({ error: "Batch not found" }); return; }
    res.json({ ok: true });
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
...
router.post("/erp/products/:id/barcodes", authenticate, requireStaff, requireStore, requirePermission("products", "manage_barcodes"), async (req: AuthRequest, res) => {
  try {
    ...
    if (!barcode || !String(barcode).trim()) { res.status(400).json({ error: "Barcode is required" }); return; }
    ...
    if (pg.code === "23505") { res.status(409).json({ error: "Ce barcode existe déjà" }); return; }
    req.log.error(err); res.status(500).json({ error: "Internal server error" });
  }
});
...
router.delete("/erp/products/:id/barcodes/:barcodeId", authenticate, requireStaff, requireStore, requirePermission("products", "manage_barcodes"), async (req: AuthRequest, res) => {
  try {
    ...
    if (isNaN(productId) || isNaN(barcodeId)) { res.status(400).json({ error: "Invalid id" }); return; }
    ...
    const result = await db.execute(sql`
      ...
      RETURNING id
    `);
    if (!result.rows.length) { res.status(404).json({ error: "Barcode not found" }); return; }
    res.json({ ok: true });
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
